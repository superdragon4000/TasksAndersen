...
class UserService {
    ...
    async changePassword(user, password) {
        const userData = await UserModel.findOne({"_id": user.userDto.id});
    ...
    async deleteUser(user, refreshToken) {
        await UserModel.deleteOne({"_id": user.userDto.id});
        await taskModel.deleteMany({"user": user.userDto.id});
